...
# Parameters
t = 0.5     # Hopping
Nwn = 1024   # Num of freq: 1024 is recommended

U_max = 3.5
U_list = np.arange(2.5, U_max, 0.1)   # Interaction strength
U_print = np.arange(2.5, U_max, 0.1)  # Values when obs should be computed
beta_list = np.arange(5, 100, 10)     # Inverse of temperature
beta_print = np.arange(5, 100, 10)

#U_list = U_print = [0.5, 1, 1.5, 2, 2.5]
beta_list = beta_print = [100]
...
for beta in beta_list:
    ...
    for U in U_list:
        ...
        if U in U_print and beta in beta_print:
            ...
            print("T="+f'{1/beta:.2f}'+"\tU="+f'{U:.3f}'+"\tG_w0.im="+f'{G_iwn_up[0].imag:.3f}')
...
for i in range(len(beta_print)):
    ...
    for j in range(len(U_print)):
        ...
        plt.legend()
        plt.savefig("./figures/Gwn/Gwn_beta="+str(beta)+"_U="+f'{U:.3f}'+".png")
        plt.close()
        ...
        plt.legend()
        plt.savefig("./figures/Gtau/Gtau_beta="+str(beta)+"_U="+f'{U:.3f}'+".png")
        plt.close()
